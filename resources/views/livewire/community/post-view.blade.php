<div class="bg-white min-h-screen pb-32 animate-fade-in-up">
    ...
    <div class="px-4 md:px-6 pt-6 pb-4 flex items-start justify-between">
        <div class="flex items-center gap-3 md:gap-4">
            ...
            {{-- Clickable Avatar --}}
            <a href="{{ route('profile.public', $authorUsername) }}" class="w-12 h-12 rounded-full shrink-0 shadow-sm hover:ring-2 hover:ring-red-200 transition-all overflow-hidden border border-gray-100 bg-gray-50">
                <img src="{{ $photoUrl }}" alt="{{ $authorName }}" class="w-full h-full object-cover">
            </a>
            
            <div class="flex flex-col justify-center">
                {{-- Clickable Name --}}
                <a href="{{ route('profile.public', $authorUsername) }}" class="font-black text-gray-900 text-[15px] md:text-base leading-tight flex items-center gap-1.5 hover:text-red-600 transition-colors cursor-pointer group">
                    {{ $authorName }}
                    @if($post->is_featured)
                        <svg class="w-4 h-4 text-yellow-500 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                    @endif
                </a>
