        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div class="xl:col-span-4 space-y-6">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                    ...
                    <div class="mb-5" x-data="{
                        ...
                        <textarea x-ref="editor" wire:model="description" rows="4" class="w-full rounded-xl border-gray-200 bg-gray-50 text-sm p-3 resize-y focus:bg-white focus:ring-orange-500 focus:border-orange-500 transition-colors" placeholder="Evaluation instructions or details..."></textarea>
                    </div>
                ...
                {{-- THE E-CERTIFICATE MASTER SUITE (Accordion) --}}
                <div x-data="{ expanded: false }" class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div x-show="expanded" x-collapse x-cloak>
                        <div class="p-6 border-t border-gray-100 space-y-6 bg-gray-50/30">
                            ...
                            {{-- INTERACTIVE PREVIEW CANVAS (Mobile & Drag/Resize Supported) --}}
                                ...
                                <div x-data="{
                                        ...
                                        handleMove(e) {
                                            ...
                                            if (this.isResizing) {
                                                let clientX = e.touches ? e.touches[0].clientX : e.clientX;
                                                let clientY = e.touches ? e.touches[0].clientY : e.clientY;
                                                // Calculate distance dragged to scale the font size
                                    ...
                                    <div class="w-full bg-gray-100 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center overflow-hidden p-4 relative group">

                                        <div class="relative inline-block select-none cursor-crosshair shadow-md touch-none"
                                            ...
                                            <div @mousedown="startDrag($event)" @touchstart.prevent="startDrag($event)"
                                                ...
                                                <div class="relative inline-block">
                                                    {{-- REMOVED bg-white/40, backdrop-blur-sm, and shadow-sm classes here --}}
            ...
            <div class="xl:col-span-8 relative">
                ...
                <div class="sticky top-6 z-[100] flex justify-center mb-8 pointer-events-none px-4 sm:px-0">
                    ...
                    {{-- WRAPPER: Controls the width of both the button and the dropdown --}}
                ...
                <div
                    class="bg-transparent min-h-[400px] space-y-4 pb-20"
                    x-data="{
                        initSortable() {
                    ...
                    @foreach($questions as $index => $question)
                        ...
                        <div data-sort-id="{{ $question['temp_id'] }}" wire:key="q-{{ $question['temp_id'] }}">
                            ...
                            @if($question['type'] === 'page_break')
                                <div class="group relative bg-red-50/80 border {{ $isActive ? 'border-red-400 shadow-md ring-2 ring-red-400/20' : 'border-red-100 hover:border-red-200 shadow-sm' }} rounded-2xl p-3 flex items-center justify-between transition-all cursor-pointer" wire:click="setActiveQuestion({{ $index }})">

                                    <div class="drag-handle w-10 flex items-center justify-center cursor-move text-red-300 hover:text-red-500">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16"></path></svg>
                                    </div>

                                    <div class="flex-1 flex flex-col sm:flex-row sm:items-center gap-4 px-2">
                                        ...
                                        @if($isActive)
                                            <div class="flex items-center gap-3">
                                                ...
                                                <select wire:model="questions.{{ $index }}.options.0.jump" class="text-sm font-medium border-red-200 rounded-xl py-2 px-3 bg-white text-gray-700 shadow-sm focus:ring-red-500 focus:border-red-500 w-48 cursor-pointer">
                                                    <option value="">Continue to next page</option>
                                                    <option value="submit">Submit Form</option>
                                                    @foreach($this->sections as $section)
                                                        @if($section['order'] > $question['order'])
                                                            <option value="{{ $section['id'] }}">Go to: {{ Str::limit($section['title'], 20) }}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <button wire:click.stop="removeQuestion({{ $index }})" class="text-red-300 hover:text-red-600 p-2 transition-colors">
                            ...
                            @else
                                @php
                                    ...
                                    $bgClass = $isSection ? 'bg-orange-50/30' : 'bg-white';
                                    $borderClass = $isActive ? 'border-orange-400 shadow-md ring-2 ring-orange-500/20' : 'border-gray-200 hover:border-gray-300 shadow-sm';
                                @endphp

                                <div class="group relative {{ $bgClass }} border {{ $borderClass }} rounded-2xl p-5 sm:p-6 transition-all duration-200 cursor-text" wire:click="setActiveQuestion({{ $index }})">

                                    {{-- Active Indicator Stripe --}}
                                    @if($isActive) <div class="absolute left-0 top-4 bottom-4 w-1.5 bg-orange-500 rounded-r-full"></div> @endif

                                    {{-- DRAG HANDLE --}}
                                    <div class="drag-handle absolute left-0 top-0 bottom-0 w-10 flex flex-col items-center justify-center cursor-move z-10 text-gray-300 hover:text-gray-500 hover:bg-gray-100 transition-colors rounded-l-2xl">
                                    ...
                                    <div class="pl-8" x-data="{
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                                            ...
                                            <div class="flex flex-col gap-3 flex-1">
                                                ...
                                                @if($isSection)
                                                    {{-- SECTION TITLE --}}
                                                    <textarea x-ref="qText" @focus="activeField = 'qText'" wire:model="questions.{{ $index }}.question_text" class="w-full text-xl font-black text-gray-800 border-0 bg-transparent placeholder-orange-300 focus:ring-0 p-0 resize-y" rows="1" placeholder="Type Section Title"></textarea>
                                                @else
                                        ...
                                        @if(!$isSection)
                                            {{-- QUESTION TITLE --}}
                                            <textarea x-ref="qText" @focus="activeField = 'qText'" wire:model="questions.{{ $index }}.question_text" class="w-full text-base font-semibold border-0 border-b-2 border-transparent hover:border-gray-200 focus:border-orange-500 focus:ring-0 bg-transparent transition-colors p-0 mb-3 resize-y" rows="1" placeholder="Enter your question..."></textarea>
                                            ...
                                            @if($isActive || $question['description'] || $question['image_path'] || (isset($question['new_image']) && $question['new_image']))
                                                ...
                                                <textarea x-ref="qDesc" @focus="activeField = 'qDesc'" wire:model="questions.{{ $index }}.description" class="w-full text-sm text-gray-500 border-0 border-b-2 border-transparent hover:border-gray-100 focus:border-orange-300 focus:ring-0 bg-transparent transition-colors p-0 mb-4 placeholder-gray-300 resize-y" rows="1" placeholder="Add helper text or a description..."></textarea>
                                        ...
                                        @else
                                            {{-- SECTION DESCRIPTION --}}
                                            <textarea x-ref="qDesc" @focus="activeField = 'qDesc'" wire:model="questions.{{ $index }}.description" class="w-full text-sm text-gray-500 border-0 bg-transparent placeholder-orange-300/60 focus:ring-0 p-0 resize-y" rows="2" placeholder="Provide context or instructions for this section..."></textarea>
                                        @endif
                                            ...
                                            <div class="pl-2 border-l-2 border-gray-100 space-y-2.5 mt-4">
                                                @foreach($question['options'] as $optIndex => $opt)
                                                    <div class="flex flex-col gap-1.5">
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-3 h-3 border-2 border-gray-300 {{ $question['type'] === 'checkbox' ? 'rounded-sm' : 'rounded-full' }}"></div>
                                                            ...
                                                            <button wire:click.stop="removeOption({{ $index }}, {{ $optIndex }})" class="text-gray-300 hover:text-red-500 p-1 transition-colors">
                                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                                            </button>
                                                        </div>
                                                        @if($question['type'] === 'radio' && is_array($opt) && $isActive)
                                                            <div class="flex items-center gap-2 pl-6">
                                                                <svg class="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path></svg>
                                                                <select wire:model="questions.{{ $index }}.options.{{ $optIndex }}.jump" class="text-xs font-medium border-gray-200 rounded-lg py-1 px-2 bg-gray-50 hover:bg-white text-gray-600 focus:ring-orange-500 focus:border-orange-500 transition-colors cursor-pointer">
                                                                    <option value="">Continue to next question</option>
                                                                    <option value="submit">Submit Form</option>
                                                                    @foreach($this->sections as $section)
                                                                        @if($section['order'] > $question['order']) <option value="{{ $section['id'] }}">Go to: {{ Str::limit($section['title'], 20) }}</option> @endif
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                        @endif
                                        ...
                                        @elseif($question['type'] === 'likert')
                                            <div class="bg-gray-50 rounded-xl p-3 border border-gray-100 mt-4">
                                                <div class="grid grid-cols-5 gap-2">
                                                    @foreach($question['options'] as $optIndex => $option)
                                                        <input type="text" wire:model="questions.{{ $index }}.options.{{ $optIndex }}" class="text-xs font-medium text-center border-gray-200 focus:border-orange-500 focus:ring-orange-500 rounded-lg p-2 shadow-sm">
                                                    @endforeach
                                                </div>
                                            </div>
                                        @endif
