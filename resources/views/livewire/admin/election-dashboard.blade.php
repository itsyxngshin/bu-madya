<div class="max-w-7xl mx-auto py-8 px-4 font-sans pb-32">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <button wire:click="$set('showTestModal', true)" class="px-5 py-2.5 bg-gray-900 hover:bg-orange-600 text-white font-bold rounded-xl shadow-md transition-colors whitespace-nowrap flex items-center gap-2">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="p-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div class="w-10 h-10 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center font-bold">{{ substr($candidate->display_name ?? $candidate->user->name ?? '?', 0, 1) }}</div>
                                    @endif
                                    <div>
                                        <p class="font-black text-gray-900">{{ $candidate->display_name ?? $candidate->user->name ?? 'Guest/Unknown' }}</p>
