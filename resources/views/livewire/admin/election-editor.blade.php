<div class="max-w-5xl mx-auto py-8 md:py-12 px-4 sm:px-6 font-sans pb-32 animate-fade-in-up">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        ...
        @if($electionRecord)
            <button wire:click="confirmWipe" class="shrink-0 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 font-bold text-xs uppercase tracking-widest rounded-xl border border-red-200 transition-colors flex items-center gap-2">
    ...
    <div class="flex items-center gap-4 md:gap-8 border-b border-gray-200 mb-8 overflow-x-auto no-scrollbar pb-2">
        <button wire:click="$set('activeTab', 'details')" class="px-2 py-2 font-black text-sm whitespace-nowrap transition-colors {{ $activeTab === 'details' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600' }}">General Details</button>
        <button wire:click="$set('activeTab', 'timeline')" class="px-2 py-2 font-black text-sm whitespace-nowrap transition-colors {{ $activeTab === 'timeline' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600' }}">Timeline</button>
        <button wire:click="$set('activeTab', 'positions')" class="px-2 py-2 font-black text-sm whitespace-nowrap transition-colors {{ $activeTab === 'positions' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600' }}">Positions</button>
        <button wire:click="$set('activeTab', 'parties')" class="px-2 py-2 font-black text-sm whitespace-nowrap transition-colors {{ $activeTab === 'parties' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600' }}">Parties & Slates</button>
    </div>
        ...
        <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-200 animate-fade-in">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 mb-2">
                    ...
                    <div class="w-full h-48 md:h-64 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-300 flex flex-col items-center justify-center overflow-hidden relative group">
                        @if ($cover_photo)
                        ...
                        @else
                            ...
                            <span class="text-xs font-bold text-gray-400">Click to upload banner (16:9 recommended)</span>
                        @endif
                        <input type="file" wire:model="cover_photo" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                ...
                <div class="md:col-span-2">
                    ...
                    <input wire:model="title" type="text" placeholder="e.g. 2026 Supreme Student Council Elections" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea wire:model="description" rows="3" placeholder="Provide context or instructions for the voters..." class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm resize-none shadow-sm"></textarea>
                ...
                <div>
                    ...
                    <select wire:model="type" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                ...
                <div>
                    ...
                    <div class="flex items-center h-11">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <span class="ml-3 text-sm font-bold text-gray-700">Allow Guest / Unauthenticated Voting</span>
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-200 animate-fade-in">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input wire:model="application_start" type="datetime-local" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                ...
                <div>
                    ...
                    <input wire:model="application_end" type="datetime-local" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                ...
                <div>
                    ...
                    <input wire:model="voting_start" type="datetime-local" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                ...
                <div>
                    ...
                    <input wire:model="voting_end" type="datetime-local" class="w-full bg-gray-50 border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                    @error('voting_end') <span class="text-[10px] text-red-500 font-bold block mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="md:col-span-2 mt-4 p-5 bg-orange-50 border border-orange-200 rounded-2xl">
                    ...
                    <input wire:model="results_release" type="datetime-local" class="w-full md:w-1/2 bg-white border border-orange-200 focus:border-orange-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
    ...
    @if($activeTab === 'positions')
        <div class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-200 animate-fade-in">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 border-b border-gray-100 pb-4 gap-4">
                ...
                <button type="button" wire:click="addPosition" class="shrink-0 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 px-5 py-2.5 rounded-xl transition-colors border border-blue-200 shadow-sm">
                    + Add Position
                </button>
            </div>
                ...
                @foreach($positions as $index => $pos)
                    <div class="flex flex-col md:flex-row items-start md:items-center gap-4 bg-gray-50 p-5 rounded-2xl border border-gray-200 relative group transition-colors hover:border-gray-300">
                        ...
                        <div class="flex-1 w-full min-w-0">
                            ...
                            <input wire:model="positions.{{ $index }}.title" type="text" placeholder="e.g. President, Senator" class="w-full bg-white border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                        ...
                        <div class="w-full md:w-32 shrink-0">
                            ...
                            <input wire:model="positions.{{ $index }}.max_winners" type="number" min="1" class="w-full bg-white border border-gray-200 focus:border-blue-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm text-center">
                            @error('positions.'.$index.'.max_winners') <span class="text-[10px] text-red-500 font-bold block mt-1">{{ $message }}</span> @enderror
                        </div>

                        <button type="button" wire:click="removePosition({{ $index }})" class="absolute top-4 right-4 md:relative md:top-auto md:right-auto md:mt-5 text-gray-400 hover:text-red-500 bg-white p-2.5 rounded-xl border border-gray-200 shadow-sm transition-colors shrink-0 group-hover:border-red-200">
    ...
    <div class="fixed bottom-0 left-0 right-0 z-50 p-4 md:p-6 bg-gradient-to-t from-white via-white/90 to-transparent pointer-events-none">
        <div class="max-w-5xl mx-auto flex justify-end pointer-events-auto">
            <button wire:click="saveElection" wire:loading.attr="disabled" class="px-8 py-4 bg-gray-900 hover:bg-blue-600 text-white font-black rounded-[1.5rem] shadow-xl transition-all transform active:scale-95 flex items-center gap-3">
        ...
        <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-[2rem] p-8 max-w-lg w-full shadow-2xl animate-fade-in-up">
                ...
                <div class="mb-6">
                    ...
                    <input wire:model="adminPassword" type="password" class="w-full bg-gray-50 border border-gray-200 focus:border-red-500 rounded-xl px-4 py-3 text-sm font-bold shadow-sm">
                    @error('adminPassword') <span class="text-[10px] text-red-500 font-bold block mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="flex justify-end gap-3">
                    <button wire:click="$set('showWipeModal', false)" class="px-5 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">Cancel</button>
                    <button wire:click="executeWipe" class="px-5 py-2.5 text-sm font-black text-white bg-red-600 hover:bg-red-700 rounded-xl shadow-lg transition-colors">Confirm & Wipe Data</button>
                </div>
            </div>
        </div>
    @endif
