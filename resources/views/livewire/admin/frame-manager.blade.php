<div class="p-6 max-w-7xl mx-auto min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @forelse($frames as $frame)
            @php
                // Safely parse JSON into paths for the preview grid
                $rawVars = is_string($frame->frame_images) ? json_decode($frame->frame_images, true) : ($frame->frame_images ?? []);
                $images = array_map(function($var) {
                    return is_array($var) ? ($var['path'] ?? '') : $var;
                }, $rawVars);
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group"
                    ...
                    <div class="px-4 pt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
                        @foreach($images as $index => $img)
                            ...
                            <button @click="activePreview = '{{ $imgUrl }}'"
                                ...
                                <img src="{{ $imgUrl }}" class="w-full h-full object-contain p-0.5">
                            </button>
                        @endforeach
                    </div>
                @endif
                ...
                <div class="p-4 flex-1 flex flex-col justify-between">
                    ...
                    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm" x-data @keydown.escape.window="$wire.cancelEdit()">
                        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in-up max-h-[90vh] flex flex-col">
                            ...
                            <div class="p-6 overflow-y-auto space-y-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1.5 flex justify-between">
                                            <span>Default Official Caption</span>
                                            <span class="text-gray-400 normal-case tracking-normal text-[9px]">Use [Brackets] for dynamic inputs</span>
                                        </label>
                                ...
                                <div class="border-t border-gray-100 pt-5">
                                    ...
                                    <div class="space-y-3 mb-4">
                                        @foreach($editVariations as $index => $variation)
                                            <div class="flex flex-col sm:flex-row items-start gap-4 bg-gray-50 p-3 rounded-xl border border-gray-100 transition-all hover:border-gray-200 hover:shadow-sm">

                                                {{-- Image Preview & Controls --}}
                                                <div class="flex flex-col gap-2 shrink-0 w-full sm:w-auto items-center sm:items-start">
                                                    <div class="w-20 h-20 bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAMUlEQVQ4T2NkYNgBxVD8nwEPsOEHMBqNhsFhAAfLwcAAYf///z8DHgZQDw1DDEAGDAAASgIdX/3i4QAAAABJRU5ErkJggg==')] bg-repeat rounded-lg overflow-hidden border border-gray-200">
                                                        <img src="{{ asset('storage/' . $variation['path']) }}" class="w-full h-full object-contain p-1">
                                                    </div>

                                                    <div class="flex items-center gap-1 w-full justify-center">
                                                        <button wire:click.prevent="moveVariationUp({{ $index }})" @if($index === 0) disabled @endif class="p-1 rounded-md text-gray-400 hover:bg-white hover:text-gray-900 disabled:opacity-30 transition-all">
                                                        ...
                                                        <button wire:click.prevent="moveVariationDown({{ $index }})" @if($index === count($editVariations) - 1) disabled @endif class="p-1 rounded-md text-gray-400 hover:bg-white hover:text-gray-900 disabled:opacity-30 transition-all">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                                                        </button>
                                                        <button wire:click.prevent="removeVariation({{ $index }})" class="p-1 rounded-md text-gray-400 hover:text-red-500 transition-all ml-auto">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                        </button>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="flex-1 w-full space-y-2">
                                                    <input wire:model="editVariations.{{ $index }}.label" type="text" placeholder="Label (e.g., Orange Theme)" class="w-full rounded-lg border-gray-200 text-xs focus:ring-orange-500 focus:border-orange-500 text-gray-700">
                                                    @error("editVariations.{$index}.label") <span class="text-[10px] text-red-500 font-bold block">{{ $message }}</span> @enderror

                                                    <textarea wire:model="editVariations.{{ $index }}.caption" rows="2" placeholder="Specific variation caption (leave blank to use default)" class="w-full rounded-lg border-gray-200 text-xs focus:ring-orange-500 focus:border-orange-500 text-gray-700"></textarea>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
