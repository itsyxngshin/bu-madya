<div class="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-6 font-sans text-gray-900 pb-24"
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-5 sm:mb-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                @if(auth()->user()->role?->role_name === 'administrator' && $evaluation->creator)
                    <span class="w-max px-2.5 py-1 bg-orange-50 border border-orange-200 text-orange-700 text-[10px] uppercase tracking-widest font-bold rounded-full shadow-sm flex items-center gap-1">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-3">
                ...
                @if($evaluation->certificate_template)
                    ...
                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 text-green-700 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-lg border border-green-200 shadow-sm">
                        ...
                        {{ $issuedCerts }} / {{ $totalResponsesCount }} Certificates Sent
                    </span>
                @endif
            </div>
        </div>

        {{-- EDGE-TO-EDGE MOBILE TABS --}}
        <div class="flex gap-6 border-b border-gray-200 mb-6 sm:mb-8 overflow-x-auto whitespace-nowrap -mx-3 px-3 sm:mx-0 sm:px-0 [&::-webkit-scrollbar]:hidden">
        ...
        @if (session()->has('error'))
            <div class="mb-6 bg-red-50 text-red-700 px-4 py-3 rounded-xl text-xs font-bold border border-red-200 animate-fade-in-up">
                {{ session('error') }}
            </div>
        @endif
        ...
        @if($tab === 'summary')
            ...
            <div class="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-100 rounded-2xl p-5 sm:p-6 shadow-sm mb-6 sm:mb-8 relative overflow-hidden animate-fade-in-up">
                <div class="absolute -right-6 -top-6 text-purple-500/10 hidden sm:block">
                ...
                <div class="relative z-10">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        <div>
                            <h3 class="text-sm font-black text-purple-900 uppercase tracking-widest flex items-center gap-2">
                                <svg class="w-5 h-5 text-purple-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                Gemini AI Executive Summary
                            </h3>
                            <p class="text-[11px] sm:text-xs text-purple-700 font-medium mt-1 leading-snug">Deep analysis of quantitative scores and qualitative written feedback.</p>
                        </div>

                        @if(!$aiReport)
                            <button wire:click="generateAIInsights" wire:loading.attr="disabled" class="w-full sm:w-auto justify-center px-5 py-3 sm:py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold uppercase tracking-widest rounded-xl shadow-md transition flex items-center gap-2 group">
                    ...
                    @if($aiReport)
                        <div class="mt-5 bg-white/60 p-4 sm:p-5 rounded-xl border border-purple-100">
                        ...
                        <div class="mt-4 flex justify-end">
                            <button wire:click="generateAIInsights" class="text-[10px] font-bold uppercase tracking-widest text-purple-500 hover:text-purple-700 transition flex items-center gap-1 p-2 -mr-2">
            ...
            @if($synthesisReport)
                <div class="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-5 sm:p-6 shadow-sm mb-6 sm:mb-8 relative overflow-hidden animate-fade-in-up">
                    <div class="absolute -right-6 -top-6 text-indigo-500/10 hidden sm:block">
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="text-indigo-900 leading-relaxed text-xs sm:text-sm font-medium prose prose-strong:text-indigo-700 max-w-none">
            ...
            {{-- LIKERT HTML TABLE WITH SUBTOTALS (Dynamic Per-Section Scales) --}}
            @php
                $groupedSections = [];
                ...
                foreach($evaluation->questions->sortBy('order') as $question) {
                    if ($question->type === 'section') {
                        ...
                        if (empty($groupedSections[$currentSectionIndex]['options'])) {
                            $opts = collect($question->options)->map(fn($opt) => is_array($opt) ? ($opt['text'] ?? '') : $opt)->toArray();
                            $groupedSections[$currentSectionIndex]['options'] = $opts;
                            $groupedSections[$currentSectionIndex]['totals'] = array_fill(0, count($opts), 0);
                        }

                        $stat = $stats[$question->id] ?? null;
                        if ($stat) {
                            foreach($groupedSections[$currentSectionIndex]['options'] as $idx => $lbl) {
            ...
            @if(count($groupedSections) > 0)
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6 sm:mb-8 animate-fade-in-up">
                    <div class="p-4 sm:p-5 border-b border-gray-200 bg-gray-50/80">
                        <h3 class="text-base sm:text-lg font-black text-gray-900 flex items-center gap-2">
                            <svg class="w-5 h-5 text-orange-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            Overall Likert Results
                        </h3>
                        <p class="text-[11px] sm:text-xs text-gray-500 mt-1">Aggregated sentiment analysis with dynamic section sub-tabulations.</p>
                    </div>

                    {{-- Mobile Optimized Table Wrapper --}}
                    <div class="overflow-x-auto custom-scrollbar -mx-px">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($groupedSections as $section)
                                    ...
                                    <tr class="bg-white border-b border-gray-200 text-[10px] uppercase tracking-widest text-gray-500 font-bold">
                                        ...
                                        <th class="p-3 sm:p-4 whitespace-nowrap min-w-[200px] max-w-[250px] bg-gray-50/90 backdrop-blur-sm sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Evaluation Criteria</th>
                                        @foreach($section['options'] as $optionLabel)
                                            <th class="p-3 sm:p-4 text-center whitespace-nowrap bg-gray-50/50">{{ $optionLabel }}</th>
                                        @endforeach
                                        <th class="p-3 sm:p-4 text-center bg-gray-50/50">Score</th>
                                    </tr>
                                    ...
                                    @foreach($section['questions'] as $question)
                                        ...
                                        <tr class="group hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="p-3 sm:p-4 text-[13px] sm:text-sm text-gray-900 font-medium pl-4 sm:pl-6 sticky left-0 bg-white group-hover:bg-gray-50 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] transition-colors break-words whitespace-normal min-w-[200px] max-w-[250px]">
                                                {{ $qText }}
                                            </td>
                                            @foreach($section['options'] as $index => $optionLabel)
                                                <td class="p-3 sm:p-4 text-center font-bold text-sm text-gray-600">
                                                    {{ $stat['breakdown'][$index] ?? 0 }}
                                                </td>
                                            @endforeach
                                            <td class="p-3 sm:p-4 text-center">
                                                <span class="px-2 py-1 rounded-lg border text-xs font-black {{ $colorClass }} shadow-sm">
                                                    {{ number_format($avg, 2) }}
                                                </span>
                                            </td>
                                        </tr>
                                    ...
                                    <tr class="bg-gray-50/80 border-t-2 border-gray-200">
                                        <td class="p-3 sm:p-4 text-[10px] font-black text-gray-700 text-right uppercase tracking-widest sticky left-0 bg-gray-50/90 backdrop-blur-sm z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                                            Section Subtotal
                                        </td>
                                        @foreach($section['totals'] as $total)
                                            <td class="p-3 sm:p-4 text-center font-black text-sm text-gray-800">
                                                {{ $total }}
                                            </td>
                                        @endforeach
                                        <td class="p-3 sm:p-4 text-center">
            ...
            <div class="space-y-4 sm:space-y-6">
                @foreach($evaluation->questions->sortBy('order') as $index => $question)
                    ...
                    @if($question->type === 'section')
                        <div class="pt-6 sm:pt-8 pb-2 border-b-2 border-orange-100">
                            <div class="text-base sm:text-lg font-black text-orange-600 uppercase tracking-tight prose prose-sm max-w-none prose-p:my-0 prose-a:text-orange-600">
                    ...
                    @elseif($question->type === 'likert')
                        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-200">
                            <div class="font-bold text-gray-900 mb-5 sm:mb-6 prose prose-sm max-w-none prose-p:mt-0 prose-p:mb-2 prose-a:text-orange-600">
                                {!! \Illuminate\Support\Str::markdown($question->question_text ?? '', ['html_input' => 'strip']) !!}
                            </div>
                            <div class="flex flex-col lg:flex-row gap-6 md:gap-8">
                                <div class="lg:w-1/3 flex flex-col justify-center items-center bg-gray-50 rounded-xl p-5 sm:p-6 border border-gray-100 text-center">
                                    <span class="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Average Rating</span>
                                    <div class="text-4xl sm:text-5xl font-black text-gray-900 mb-1">{{ $stat['average'] ?? '0.0' }}</div>
                                    ...
                                    <span class="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase">{{ $stat['count'] ?? 0 }} Responses</span>
                                </div>
                                <div class="lg:w-2/3">
                                    @if(isset($stat['count']) && $stat['count'] > 0)
                                        <div class="h-48 sm:h-64 w-full relative" wire:ignore>
                                            <canvas x-data="{
                                                init() {
                                                    new Chart(this.$el, {
                                                        type: 'bar',
                    ...
                    @elseif(in_array($question->type, ['radio', 'dropdown']))
                        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-200">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-5 sm:mb-6">
                                ...
                                <span class="bg-gray-100 text-gray-500 text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded uppercase w-max">
                                        ...
                                        @foreach($question->options as $option)
                                            ...
                                            <div class="bg-gray-50 rounded-xl p-3 border border-gray-100 relative overflow-hidden">
                                                ...
                                                <div class="flex justify-between items-center relative z-10">
                                                    <span class="font-bold text-gray-700 text-xs sm:text-sm truncate pr-2" title="{{ $label }}">{{ $label }}</span>
                                                    <span class="font-black text-gray-900 text-sm">{{ $count }}</span>
                                                </div>
                                                <span class="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase">{{ round($percent) }}%</span>
                                            </div>
                                        @endforeach
                                    </div>
                                    <div class="h-48 sm:h-64 w-full relative" wire:ignore>
                    ...
                    @elseif($question->type === 'checkbox')
                        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-200">
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-5 sm:mb-6">
                                ...
                                <span class="bg-blue-50 text-blue-600 text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded uppercase w-max">Multi-Select</span>
                            </div>
                                        ...
                                        @foreach($question->options as $option)
                                            ...
                                            <div class="relative">
                                                <div class="flex justify-between items-center mb-1 text-xs sm:text-sm">
                                                    <span class="font-bold text-gray-700 truncate pr-2">{{ $label }}</span>
                                                    <span class="font-bold text-gray-900 shrink-0">{{ $count }} <span class="text-gray-400 text-[10px] sm:text-xs font-normal">({{ round($percent) }}%)</span></span>
                                                </div>
                    ...
                    @else
                        <div class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-200">
                            <div class="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4">
                                <div class="font-bold text-gray-900 prose prose-sm max-w-none prose-p:mt-0 prose-p:mb-2 prose-a:text-orange-600">
                                    {!! \Illuminate\Support\Str::markdown($question->question_text ?? '', ['html_input' => 'strip']) !!}
                                </div>
                                <span class="bg-gray-100 text-gray-500 text-[9px] sm:text-[10px] font-bold px-2 py-1 rounded uppercase w-max shrink-0">
                            ...
                            @if($question->answers->count() > 0)
                                <div class="max-h-48 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                    @foreach($question->answers->take(10) as $answer)
                                        <div class="text-xs sm:text-sm text-gray-600 bg-gray-50 p-2.5 sm:p-3 rounded-lg border border-gray-100 break-words">
        ...
        @elseif($tab === 'individual')
            ...
            @else
                ...
                <div class="bg-white/95 backdrop-blur-md rounded-2xl p-3 sm:p-4 shadow-sm border border-gray-200 mb-6 flex items-center justify-between sticky top-4 z-40 drop-shadow-md">
                    ...
                    <div class="flex-1 px-2 flex flex-col items-center justify-center z-50">
                        ...
                        <div class="relative w-max mx-auto"
                            ...
                            <div x-show="open"
                                 ...
                                 class="absolute top-[calc(100%+8px)] left-1/2 transform -translate-x-1/2 w-48 sm:w-56 bg-white/95 backdrop-blur-xl border border-gray-100 rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden z-[100]"
                                 style="display: none;">

                                <div class="max-h-60 sm:max-h-72 overflow-y-auto custom-scrollbar p-2 space-y-1">
                                    @for($i = 0; $i < $totalResponsesCount; $i++)
                                        <button @click="activeIndex = {{ $i }}; open = false"
                                                type="button"
                                            ...
                                            {{-- Selected Animated Checkmark --}}
                                            <div x-show="activeIndex === {{ $i }}"
                                                 x-transition:enter="transition ease-out duration-300 transform"
                                                 x-transition:enter-start="opacity-0 scale-50 rotate-[-45deg]"
                                                 x-transition:enter-end="opacity-100 scale-100 rotate-0"
                                                 class="relative z-10 bg-white/20 p-1 rounded-full"
                                                 style="display: none;">
                                                <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                            </div>

                                            {{-- Unselected Hover Dot --}}
                                            <div x-show="activeIndex !== {{ $i }}"
                                                 class="w-1.5 h-1.5 rounded-full bg-[var(--theme)] opacity-0 group-hover/item:opacity-100 transition-all transform translate-x-2 group-hover/item:translate-x-0 relative z-10">
                                            </div>
                                        </button>
                                    @endfor
                                </div>
                    ...
                    {{-- Next Button --}}
                    <button wire:click="nextResponse" class="p-2 sm:p-3 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl transition disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center shrink-0" {{ $currentIndex === ($totalResponsesCount - 1) ? 'disabled' : '' }}>
                ...
                <div class="bg-gray-900 text-white rounded-2xl p-5 sm:p-6 shadow-md mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                        <span class="text-[9px] sm:text-[10px] text-gray-400 uppercase tracking-widest font-bold">Submitted By</span>
                        <h2 class="text-base sm:text-lg font-bold mt-1">
                    ...
                    @if($evaluation->certificate_template)
                        <div class="w-full md:w-auto flex flex-col sm:flex-row items-start sm:items-center gap-3">
                            @if($currentResponse->certificate_issued_at)
                                <div class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-green-500/20 border border-green-500/30 text-green-400 text-[9px] uppercase tracking-widest font-bold rounded-lg w-max">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    Issued {{ \Carbon\Carbon::parse($currentResponse->certificate_issued_at)->format('M d, Y') }}
                                </div>
                            @else
                                <div class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-gray-800 border border-gray-700 text-gray-400 text-[9px] uppercase tracking-widest font-bold rounded-lg w-max">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    Pending Issuance
                                </div>
                            @endif

                            <button wire:click="openIssueModal({{ $currentResponse->id }})" class="w-full sm:w-auto px-5 py-3 sm:py-2.5 {{ $currentResponse->certificate_issued_at ? 'bg-gray-800 hover:bg-gray-700 text-gray-200' : 'bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-lg shadow-orange-900/20' }} text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-xl transition flex items-center justify-center gap-2">
                    ...
                    <div class="text-left md:text-right border-t border-gray-700 pt-4 md:border-t-0 md:pt-0 w-full md:w-auto">
                        <span class="text-[9px] sm:text-[10px] text-gray-400 uppercase tracking-widest font-bold">Date & Time</span>
                        <p class="text-xs sm:text-sm mt-1">{{ $currentResponse->created_at->format('F d, Y - h:i A') }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                    @foreach($evaluation->questions->sortBy('order') as $question)
                        ...
                        @if($question->type === 'section')
                            <div class="font-bold text-gray-900 mb-2 prose prose-sm max-w-none prose-p:mt-0 prose-p:mb-2 prose-a:text-orange-600 px-4 sm:px-6 pt-5 sm:pt-6">
                                {!! \Illuminate\Support\Str::markdown($question->question_text ?? '', ['html_input' => 'strip']) !!}
                            </div>
                        @else
                            @php
                            ...
                            <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 last:border-0">
                                <div class="font-bold text-gray-900 mb-3 prose prose-sm max-w-none prose-p:mt-0 prose-p:mb-2 prose-a:text-orange-600">
                                    {!! \Illuminate\Support\Str::markdown($question->question_text ?? '', ['html_input' => 'strip']) !!}
                                </div>
                                ...
                                @elseif($question->type === 'file')
                                    <button type="button" @click="openPreview('{{ asset('storage/'.$val) }}')" class="inline-flex items-center gap-2 px-5 py-3 sm:py-2.5 bg-gray-900 text-white font-bold text-xs rounded-xl hover:bg-gray-800 shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        Preview File
                                    </button>
                                @elseif($question->type === 'checkbox')
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @if(is_array($decoded))
                                            @foreach($decoded as $item)
                                                <span class="px-3 py-1 bg-gray-100 text-gray-700 text-[11px] sm:text-xs font-bold rounded">{{ $item }}</span>
                                            @endforeach
                                        @else
                                            <span class="text-xs sm:text-sm text-gray-900">{{ $val }}</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="text-xs sm:text-sm text-gray-900 font-medium bg-gray-50/50 p-3 rounded-lg border border-gray-100 break-words">{{ $val }}</div>
                                @endif
                            </div>
                        @endif
                    @endforeach
    ...
    <div x-show="previewOpen" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-6" x-cloak>
        ...
        <div x-show="previewOpen"
            ...
            <div class="flex items-center justify-between p-3 sm:p-4 border-b border-gray-100 bg-white z-10 shrink-0">
                <h3 class="text-xs sm:text-sm font-black text-gray-900 uppercase tracking-widest flex items-center gap-2 pr-4 sm:pr-6">
                    <svg class="w-4 h-4 text-orange-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    <span class="truncate max-w-[120px] sm:max-w-none">File Preview</span>
                </h3>
                <div class="flex items-center gap-1 sm:gap-2 shrink-0">
                    <a :href="previewUrl" target="_blank" download class="px-3 py-2 sm:py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-[10px] font-bold uppercase tracking-widest rounded-lg transition flex items-center gap-1.5">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                        <span class="hidden sm:inline">Download</span>
                    </a>
                    <button @click="previewOpen = false" class="p-2 sm:p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition">
            ...
            <div class="overflow-hidden bg-gray-100/80 flex items-center justify-center p-2 sm:p-4 relative"
                ...
                <template x-if="previewType === 'other'">
                    <div class="text-center p-6 sm:p-8 max-w-sm">
                        <div class="w-16 h-16 sm:w-20 sm:h-20 bg-gray-200 text-gray-400 rounded-full flex items-center justify-center mx-auto mb-4 shadow-inner">
                        ...
                        <h4 class="text-gray-900 font-bold text-base sm:text-lg mb-1">Preview Not Available</h4>
                        <p class="text-gray-500 text-xs sm:text-sm mb-6 leading-relaxed">This file format cannot be displayed natively in the browser. You must download it to view.</p>
                        <a :href="previewUrl" download class="w-full sm:w-auto justify-center px-6 py-3 bg-gray-900 hover:bg-gray-800 text-white font-bold rounded-xl transition shadow-lg text-[10px] sm:text-xs uppercase tracking-widest inline-flex items-center gap-2">
        ...
        <div class="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-6 bg-gray-900/80 backdrop-blur-sm transition-opacity"
            ...
            <div class="relative bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden max-h-[85vh] sm:max-h-[90vh]">
                ...
                <div class="flex items-center justify-between p-4 sm:p-6 border-b border-gray-100 bg-white shrink-0">
                    <div>
                        <h3 class="text-base sm:text-xl font-black text-gray-900 leading-tight">Issue Certificate</h3>
                        <p class="text-[10px] sm:text-xs font-bold text-gray-400 mt-0.5">Verify details before sending.</p>
                    </div>
                ...
                {{-- Modal Body --}}
                <div class="p-4 sm:p-6 bg-gray-50 flex-1 overflow-y-auto custom-scrollbar">
                    <div class="space-y-3 sm:space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Participant Name</label>
                                <input type="text" wire:model="issueName" class="w-full bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 text-sm font-bold focus:ring-orange-500 focus:border-orange-500 shadow-sm">
                            ...
                            <div>
                                ...
                                <input type="email" wire:model="issueEmail" class="w-full bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 text-sm focus:ring-orange-500 focus:border-orange-500 shadow-sm" placeholder="Leave blank to skip">
                        ...
                        <div class="border-t border-gray-200 pt-3 sm:pt-4">
                            ...
                            <input type="text" wire:model="issueSubject" class="w-full bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2 text-sm font-bold focus:ring-orange-500 focus:border-orange-500 shadow-sm mb-3">

                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Email Body</label>
                            {{-- [FIXED] Reduced from 4 rows to 2 rows to save vertical screen space --}}
                            <textarea wire:model="issueBody" rows="2" class="w-full bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2 text-sm focus:ring-orange-500 focus:border-orange-500 resize-y shadow-sm"></textarea>
                        </div>
                    </div>
                </div>

                {{-- Modal Footer Actions --}}
                {{-- [FIXED] Changed to flex-row so buttons sit side-by-side on mobile instead of stacking --}}
                <div class="p-4 sm:p-6 bg-white border-t border-gray-100 flex flex-row items-center justify-between gap-2 sm:gap-4 shrink-0 pb-6 sm:pb-6">
                    <button wire:click="downloadCertificate" class="flex-1 sm:flex-none px-2 sm:px-6 py-3 sm:py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl text-[10px] sm:text-xs uppercase tracking-widest transition flex items-center justify-center gap-1.5 sm:gap-2 leading-tight">
                    ...
                    <button wire:click="sendCertificateEmail" class="flex-1 sm:flex-none px-2 sm:px-6 py-3 sm:py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-xl text-[10px] sm:text-xs uppercase tracking-widest shadow-lg transition flex items-center justify-center gap-1.5 sm:gap-2 leading-tight">
