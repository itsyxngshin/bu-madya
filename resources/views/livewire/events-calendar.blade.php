<div class="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden font-sans">
    ...
    {{-- 1. HEADER --}}
    <div class="px-4 md:px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4 bg-stone-50">
        <div>
            <h2 class="text-lg md:text-xl font-black text-gray-900 uppercase tracking-tight text-center md:text-left">
        ...
        <div class="flex items-center bg-white rounded-lg border border-gray-200 shadow-sm">
            <button wire:click="previousMonth" class="px-4 py-1.5 hover:bg-gray-100 text-gray-500 border-r border-gray-200 transition">
                &larr;
            </button>
            <button wire:click="nextMonth" class="px-4 py-1.5 hover:bg-gray-100 text-gray-500 transition">
                &rarr;
            </button>
        </div>
    </div>
    ...
    {{-- 2. CALENDAR BODY --}}
    {{-- Added 'overflow-x-auto' to enable scrolling on mobile --}}
    <div class="w-full overflow-x-auto">
        
        {{-- CONTAINER WITH MIN-WIDTH --}}
        {{-- 'min-w-[800px]' ensures the grid never gets squished below 800px width --}}
            ...
            <div style="display: grid; grid-template-columns: repeat(7, 1fr);" class="bg-gray-200 gap-px border-b border-gray-200">
                ...
                @for($day = 1; $day <= $daysInMonth; $day++)
                    ...
                    <div class="bg-white min-h-[100px] p-2 relative hover:bg-red-50 transition group flex flex-col gap-1">
                        ...
                        {{-- Events List --}}
                        <div class="flex-1 w-full min-w-0"> {{-- min-w-0 fixes flex truncation --}}
                            @if(isset($events[$day]))
                                @foreach($events[$day] as $event)
                                    <a href="{{ route('projects.show', $event['slug']) }}" 
                                       class="block w-full px-2 py-1 mb-1 rounded bg-yellow-50 border-l-2 border-yellow-400 text-[9px] font-bold text-gray-800 hover:bg-yellow-100 transition shadow-sm truncate"
                                       title="{{ $event['title'] }}">
                                        {{ $event['title'] }}
                                    </a>
                                @endforeach
                        ...
                        @if(Auth::check() && in_array(Auth::user()->role_id, [1, 2]))
                            ...
                            <a href="{{ route('projects.create', ['date' => $targetDate]) }}" 
                               class="absolute top-2 right-2 hidden group-hover:block p-1 bg-gray-100 text-gray-500 rounded hover:bg-red-600 hover:text-white transition z-10">
