    ...
    <div class="max-w-7xl mx-auto px-6 py-12">
        ...
        <div class="mb-16">
            ...
            <div class="flex flex-wrap justify-center gap-8">
                @forelse($committee->directorAssignments as $assignment)
                    @if($assignment->user)
                        <div class="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 flex items-center gap-5 w-full md:w-auto md:min-w-[350px] hover:-translate-y-1 transition duration-300">
                            ...
                            <div>
                                <h4 class="font-heading font-bold text-lg text-gray-900">{{ $assignment->user->name }}</h4>
                                {{-- Display custom title if set on assignment, otherwise fallback to standard title --}}
                                <p class="text-xs font-bold text-red-600 uppercase tracking-wider mb-1">
                                    {{ $assignment->title ?? ($assignment->director->title ?? 'Committee Head') }}
                                </p>
                                <p class="text-xs text-gray-500">{{ $assignment->user->course ?? 'Bicol University' }}</p>
                            </div>
                        </div>
                    @endif
                @empty
                    <div class="text-center text-gray-400 py-4 w-full">
                        <p class="text-xs font-bold uppercase tracking-widest">Leadership positions currently vacant</p>
                    </div>
                @endforelse
            </div>
        </div>
