<div class="max-w-5xl mx-auto py-8 md:py-12 px-4 sm:px-6 font-sans pb-40 animate-fade-in-up">
    ...
    @else
        ...
        <div class="bg-white rounded-[2.5rem] shadow-lg border border-gray-200 overflow-hidden mb-8 relative">
            ...
            <div class="px-6 md:px-10 pb-8 relative z-10 -mt-12 md:-mt-16 text-center md:text-left flex flex-col md:flex-row items-center gap-6">
                <div class="w-24 h-24 md:w-32 md:h-32 bg-white rounded-3xl shadow-xl border-[6px] border-white flex items-center justify-center rotate-3 shrink-0">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-1">Official Electronic Ballot</p>
                    <h1 class="text-3xl md:text-5xl font-black text-gray-900 tracking-tighter leading-none mb-2 break-words">
                        {{ $election->title }}
                    </h1>
                    <p class="text-sm font-bold text-gray-500">Please review your selections carefully before casting.</p>
                </div>
            </div>
        </div>

        @if(session()->has('error'))
            <div class="mb-8 bg-red-50 text-red-700 p-5 rounded-2xl border border-red-200 shadow-sm flex items-center gap-3">
        ...
        <div class="space-y-10">
            @foreach($positions as $position)
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-gray-200 relative overflow-hidden" wire:key="pos-{{ $position->id }}">
                    ...
                    <div class="bg-gray-50 border-b-[3px] border-dashed border-gray-200 px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h2 class="text-2xl font-black text-gray-900 tracking-tight uppercase font-serif">{{ $position->title }}</h2>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            ...
                            @foreach($position->candidates as $candidate)
                                ...
                                <div wire:click="toggleSelection({{ $position->id }}, {{ $candidate->id }})" 
                                     class="relative p-5 rounded-2xl border-2 transition-all duration-300 cursor-pointer group flex flex-col gap-4 
                                            {{ $isSelected ? 'border-green-500 bg-green-50/50 shadow-md transform -translate-y-1' : 'border-gray-100 bg-white hover:border-green-200 hover:bg-gray-50 hover:shadow-sm' }}">
                                    
                                    {{-- Custom Voting Oval / Checkbox --}}
                                    <div class="absolute top-5 right-5 w-7 h-7 rounded-full border-[3px] flex items-center justify-center transition-all duration-300 shadow-inner
                                                {{ $isSelected ? 'border-green-500 bg-green-500' : 'border-gray-300 bg-white group-hover:border-green-300' }}">
                                        @if($isSelected)
                                            <svg class="w-4 h-4 text-white animate-fade-in" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
                                        @endif
                                    </div>
                                    ...
                                    {{-- Info --}}
                                    <div class="flex-1">
                                        <h3 class="text-lg font-black text-gray-900 leading-tight mb-1 group-hover:text-green-700 transition-colors">{{ $candidateName }}</h3>
                                        <p class="text-xs font-bold text-gray-500">{{ $candidate->program }}</p>
                                    </div>

                                    {{-- Profile Link --}}
                                    <a href="{{ route('candidate.profile', $candidate->id) }}" target="_blank" @click.stop class="mt-auto text-[10px] font-black text-blue-500 hover:text-blue-700 uppercase tracking-widest flex items-center gap-1">
                            ...
                            <div wire:click="toggleSelection({{ $position->id }}, 'abstain')" 
                                 class="relative p-5 rounded-2xl border-2 transition-all duration-300 cursor-pointer group flex items-center gap-4 sm:col-span-full lg:col-span-1
                                        {{ $isAbstain ? 'border-orange-500 bg-orange-50/50 shadow-md' : 'border-gray-100 border-dashed bg-gray-50 hover:border-orange-300 hover:bg-orange-50/30' }}">
                                
                                <div class="w-7 h-7 rounded-full border-[3px] flex items-center justify-center transition-all duration-300 shrink-0
                                            {{ $isAbstain ? 'border-orange-500 bg-orange-500' : 'border-gray-300 bg-white group-hover:border-orange-300' }}">
                                    @if($isAbstain)
                                        <svg class="w-4 h-4 text-white animate-fade-in" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
                                    @endif
                                </div>
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Leave position blank</p>
                                </div>
                            </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 p-4 md:p-6 bg-gradient-to-t from-white via-white to-transparent pointer-events-none">
