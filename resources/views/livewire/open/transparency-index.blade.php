<div class="min-h-screen bg-stone-50 font-sans text-gray-900 relative overflow-x-hidden">
    ...
    {{-- 2. HERO HEADER --}}
    <header class="relative z-10 pt-24 md:pt-32 pb-8 md:pb-12 px-6">
        <div class="max-w-7xl mx-auto text-center">
            ...
            <h1 class="font-heading font-black text-3xl md:text-6xl text-gray-900 mb-4 md:mb-6 drop-shadow-sm">
                Transparency <span class="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-yellow-500">Board</span>
            </h1>
            <p class="text-gray-500 text-sm md:text-lg max-w-2xl mx-auto leading-relaxed">
    ...
    {{-- 3. FILTERS & CONTROLS --}}
    <div class="sticky top-16 md:top-20 z-30 bg-stone-50/90 backdrop-blur-xl border-y border-gray-200/50 py-3 md:py-4 mb-8 md:mb-12 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 md:px-6 flex flex-col lg:flex-row justify-between items-center gap-3 md:gap-4">
            
            {{-- A. CATEGORIES: MOBILE VIEW (Dropdown) --}}
            <div class="block lg:hidden w-full relative">
                <select wire:model.live="filter_category" 
                        class="w-full appearance-none pl-4 pr-10 py-2.5 rounded-xl border-gray-200 bg-white text-sm font-bold text-gray-700 shadow-sm focus:ring-red-500 focus:border-red-500">
            ...
            {{-- B. CATEGORIES: DESKTOP VIEW (Buttons) --}}
            <div class="hidden lg:flex gap-2 overflow-x-auto pb-2 lg:pb-0 max-w-full no-scrollbar w-full lg:w-auto">
            ...
            {{-- C. SECONDARY FILTERS (Year & Search) --}}
            <div class="flex gap-2 md:gap-3 w-full lg:w-auto">
                {{-- Year Dropdown --}}
                <div class="relative min-w-[120px] md:min-w-[140px]">
                    <select wire:model.live="filter_year" class="w-full appearance-none pl-3 md:pl-4 pr-8 md:pr-10 py-2.5 rounded-xl border-gray-200 bg-white text-xs md:text-sm focus:ring-red-500 focus:border-red-500 shadow-sm font-bold text-gray-600 cursor-pointer hover:border-gray-300 transition">
                ...
                <div class="relative w-full lg:w-64 group">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search..." 
                           class="w-full pl-9 md:pl-10 pr-4 py-2.5 rounded-xl border-gray-200 bg-white text-xs md:text-sm focus:ring-red-500 focus:border-red-500 shadow-sm transition">
    ...
    {{-- 4. DOCUMENTS GRID --}}
    <div class="relative z-10 max-w-7xl mx-auto px-4 md:px-6 pb-24">
        ...
        <div wire:loading.remove>
            @if($documents->count() > 0)
                {{-- Updated Grid: 1 col mobile, 2 col tablet, 3 col desktop --}}
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    @foreach($documents as $doc)
                        <div class="group bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative flex flex-col h-full">
                            ...
                            {{-- Card Header --}}
                            <div class="flex justify-between items-start mb-3 md:mb-4 pl-3">
                                {{-- Category Badge --}}
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg 
                                    bg-{{ $doc->category->color ?? 'gray' }}-50 
                                    text-{{ $doc->category->color ?? 'gray' }}-600 
                                    text-[9px] md:text-[10px] font-black uppercase tracking-widest border border-{{ $doc->category->color ?? 'gray' }}-100">
                                    {{ $doc->category->name }}
                                </span>
                                
                                {{-- Year Badge --}}
                                <span class="bg-gray-50 text-gray-500 text-[9px] md:text-[10px] font-bold px-2 py-1 rounded-lg border border-gray-200">
                                    {{ $doc->academic_year }}
                                </span>
                            </div>
                            ...
                            <div class="pl-3 flex-grow">
                                <h3 class="font-heading font-bold text-base md:text-lg text-gray-900 mb-2 leading-tight group-hover:text-red-600 transition line-clamp-2" title="{{ $doc->title }}">
                                    {{ $doc->title }}
                                </h3>
                                <div class="flex items-center gap-2 text-[10px] md:text-xs text-gray-400 mb-2 md:mb-3 font-mono">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    {{ $doc->published_date->format('M d, Y') }}
                                </div>
                                <p class="text-xs md:text-sm text-gray-500 line-clamp-2 mb-4">
                            ...
                            <div class="pl-3 pt-4 mt-auto border-t border-gray-100 flex items-center justify-between">
                                <div class="flex flex-col">
                                    <span class="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-gray-400">
                                ...
                                <button wire:click="download({{ $doc->id }})" 
                                        ...
                                        class="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-xl bg-gray-900 text-white text-[10px] md:text-xs font-bold uppercase tracking-widest hover:bg-red-600 hover:shadow-lg hover:-translate-y-0.5 transition-all shadow-md group-hover:shadow-xl disabled:opacity-50 disabled:cursor-wait">
            ...
            @else
                {{-- Empty State --}}
                <div class="flex flex-col items-center justify-center py-16 md:py-24 bg-white/50 backdrop-blur-sm rounded-[2rem] border-2 border-dashed border-gray-200">
                    <div class="w-12 h-12 md:w-16 md:h-16 bg-gray-100 rounded-full flex items-center justify-center text-gray-300 mb-4">
                    ...
                    <h3 class="text-lg md:text-xl font-bold text-gray-900 mb-1">No documents found</h3>
                    <p class="text-gray-500 text-xs md:text-sm">Try adjusting your filters or check back later.</p>
    ...
    <footer class="bg-gray-900 text-white pt-20 pb-10 border-t-8 border-red-600 relative z-20">
        {{-- ... (Keep Footer as is, maybe reduce padding slightly for mobile if desired) ... --}}
