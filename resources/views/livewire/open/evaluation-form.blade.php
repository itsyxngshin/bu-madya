                                        ...
                                        <div class="space-y-2">
                                            @foreach($question->options as $optIndex => $option)
                                                @php
                                                    ...
                                                    if (!is_array($currentVal)) $currentVal = []; // Safety check
                                                    $isChecked = in_array($label, $currentVal);
                                                @endphp
                                                {{-- [FIX] Added State to wire:key to force re-render --}}
                                                <label class="relative flex items-center p-3 rounded-xl border cursor-pointer transition-all group/option {{ $isChecked ? 'bg-orange-50 border-orange-500 shadow-sm' : 'border-gray-200 hover:bg-orange-50 hover:border-orange-200' }}">
                                                    <input type="checkbox" wire:model.live="answers.{{ $question->id }}" value="{{ $label }}" class="peer sr-only" wire:key="q-{{ $question->id }}-chk-{{ $optIndex }}-{{ $isChecked ? '1' : '0' }}">
