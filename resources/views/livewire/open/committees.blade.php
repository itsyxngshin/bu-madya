<div class="font-sans antialiased text-gray-800 bg-gray-50 selection:bg-red-500 selection:text-white">
    ...
    <div class="relative min-h-screen px-6 pb-24 mt-12 max-w-[1800px] w-[95%] mx-auto">
        ...
        {{-- ========================================== --}}
        {{-- UNIFIED COMMITTEES GRID --}}
        {{-- ========================================== --}}
        <section class="relative z-10 mb-32 mt-12">
            <div class="flex items-center gap-4 mb-10 justify-center text-center">
                <div>
                    <h2 class="font-heading text-3xl md:text-4xl font-bold text-gray-900">All Committees</h2>
                    <p class="text-sm text-gray-500 mt-1">Tap any card below to view the Committee Head</p>
                </div>
            </div>

            <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($committees as $index => $comm)
                    {{-- Alternate themes between red and green for visual variety --}}
                    @include('partials.committee-flip-card', [
                        'committee' => $comm, 
                        'theme' => $index % 2 === 0 ? 'red' : 'green' 
                    ])
                @endforeach
            </div>
        </section>
