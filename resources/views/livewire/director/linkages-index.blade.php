        ...
        <div class="relative z-10 text-center px-4 mt-16">
            ...
            <div class="inline-flex gap-8 bg-white/10 backdrop-blur-md px-8 py-4 rounded-2xl border border-white/20">
                <div class="text-center">
                    <span class="block text-2xl font-black text-white">{{ $this->stats['active_count'] }}</span>
                    <span class="text-[10px] uppercase tracking-wider text-gray-200">Active Partners</span>
                </div>
                <div class="w-px bg-white/20"></div>
                <div class="text-center">
                    <span class="block text-2xl font-black text-yellow-400">{{ $this->stats['moa_count'] }}</span>
                    <span class="text-[10px] uppercase tracking-wider text-gray-200">MOAs Signed</span>
                </div>
                <div class="w-px bg-white/20"></div>
                <div class="text-center">
                    <span class="block text-2xl font-black text-green-400">{{ $this->stats['intl_count'] }}</span>
    ...
    <div class="relative min-h-screen px-6 pb-24 mt-12 max-w-[1800px] w-[95%] mx-auto grid lg:grid-cols-12 gap-12">
        ...
        <main class="lg:col-span-8">
            <div class="flex items-center justify-between mb-8">
                ...
                class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="group bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition duration-300 relative overflow-hidden">
                    ...
                    <div class="flex items-start gap-4">
                        {{-- Logo (Falls back to placeholder if null) --}}
                        <div class="w-16 h-16 rounded-xl bg-gray-50 p-2 border border-gray-100 shrink-0 flex items-center justify-center">
                            @if($partner->logo_path)
                                <img src="{{ asset('storage/' . $partner->logo_path) }}" class="w-full h-full object-contain mix-blend-multiply">
                            @else
                                <span class="text-xs font-bold text-gray-300">{{ $partner->acronym ?? 'LOGO' }}</span>
                            @endif
                        </div>
                        ...
                        <div class="flex-grow">
                            <div class="flex justify-between items-start">
                                <h4 class="font-bold text-gray-900 leading-tight group-hover:text-red-700 transition line-clamp-1">
                                    {{ $partner->name }}
                                </h4>
                                {{-- Linkage Type --}}
                                @if($partner->type)
                                    <span class="text-[9px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full {{ $partner->type->color ?? 'bg-gray-100 text-gray-500' }}">
                                        {{ $partner->type->name }}
                                    </span>
                                @endif
                            </div>
                            ...
                            {{-- Status --}}
                            <p class="text-[10px] font-bold uppercase tracking-widest mt-1 mb-2 flex items-center gap-1 {{ $partner->status->color ?? 'text-gray-400' }}">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-lg border border-gray-100 sticky top-24">
                ...
                <div class="relative border-l-2 border-gray-200 ml-3 space-y-8 pl-6 pb-2">
                    ...
                    <div class="relative group">
                        ...
                        <p class="text-xs text-gray-500 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100 line-clamp-3">
