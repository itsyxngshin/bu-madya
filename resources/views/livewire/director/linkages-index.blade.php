<div class="min-h-screen font-sans text-gray-900 pb-20 px-3 md:px-0">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4 md:mb-8 pt-4">
        ...
        <button wire:click="create" 
           class="w-full md:w-auto px-4 py-2 bg-gray-900 text-white text-[10px] md:text-xs font-bold uppercase rounded-lg shadow-lg hover:bg-red-600 transition flex items-center justify-center gap-2 active:scale-95">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            <span>Add Partner</span>
        </button>
    </div>

    {{-- TOOLBAR: Compact Inputs --}}
    <div class="bg-white p-3 rounded-xl shadow-sm border border-gray-100 mb-4 flex flex-col md:flex-row gap-2 items-center justify-between">
        {{-- Search --}}
        <div class="relative w-full md:w-96">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search..." 
                   class="w-full pl-9 pr-3 py-1.5 border-gray-200 rounded-lg text-xs focus:ring-red-500 focus:border-red-500">
        </div>

        {{-- Filters --}}
        <div class="grid grid-cols-2 gap-2 w-full md:w-auto">
            <select wire:model.live="typeFilter" class="block w-full pl-2 pr-6 py-1.5 text-[10px] md:text-xs font-bold border-gray-200 rounded-lg bg-gray-50 focus:ring-red-500">
                <option value="">All Types</option>
                @foreach($this->types as $t) <option value="{{ $t->id }}">{{ $t->name }}</option> @endforeach
            </select>
            <select wire:model.live="statusFilter" class="block w-full pl-2 pr-6 py-1.5 text-[10px] md:text-xs font-bold border-gray-200 rounded-lg bg-gray-50 focus:ring-red-500">
    ...
    {{-- LINKAGE GRID: High Density / Smaller Cards --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition group overflow-hidden flex flex-col h-full">
            ...
            <div class="p-3 flex-1">
                ...
                <div class="flex justify-between items-start mb-2">
                    <div class="w-8 h-8 rounded-lg bg-gray-50 border border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
                        @if($linkage->logo_path)
                            <img src="{{ asset('storage/'.$linkage->logo_path) }}" class="w-full h-full object-contain">
                        @else
                            <span class="text-[9px] font-bold text-gray-400">{{ substr($linkage->name, 0, 2) }}</span>
                        @endif
                    </div>
                ...
                {{-- Name & Acronym (Smaller Text) --}}
                <h3 class="font-bold text-gray-900 text-sm leading-tight mb-0.5 line-clamp-1">{{ $linkage->name }}</h3>
                ...
                {{-- STATISTICS: Inline & Clean (Fixed "Weird Placement") --}}
                <div class="flex items-center gap-3 text-[10px] text-gray-500 bg-gray-50 p-2 rounded-lg border border-gray-100/50">
                    <div class="flex items-center gap-1.5 truncate">
                        ...
                        <span class="font-semibold text-gray-700">{{ optional($linkage->established_at)->format('M Y') ?? '-' }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5 truncate">
                        ...
                        <span class="font-semibold text-blue-600 truncate">{{ $linkage->agreementLevel->name ?? 'None' }}</span>
                    </div>
                </div>
            </div>

            {{-- Footer Actions --}}
            <div class="bg-gray-50/50 px-3 py-2 border-t border-gray-100 flex justify-between items-center">
        ...
        @empty
        <div class="col-span-full py-8 flex flex-col items-center justify-center text-gray-400 bg-white rounded-xl border border-dashed border-gray-200">
    ...
    {{-- CREATE / EDIT MODAL (Full Height Mobile) --}}
            ...
            <div class="p-5 overflow-y-auto flex-1">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea wire:model="form.description" rows="3" placeholder="Description..." class="w-full border-gray-200 rounded-lg text-xs md:text-sm py-2"></textarea>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-[9px] font-bold text-gray-400 uppercase mb-1">Start</label>
                                <input wire:model="form.established_at" type="date" class="w-full border-gray-200 rounded-lg text-xs py-2">
                            </div>
                            <div>
                                <label class="block text-[9px] font-bold text-gray-400 uppercase mb-1">End</label>
                                <input wire:model="form.expires_at" type="date" class="w-full border-gray-200 rounded-lg text-xs py-2">
                            </div>
                        </div>

                        <div>
                             <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">SDGs</label>
    ...
    <div class="fixed inset-0 z-50 flex items-end md:items-center justify-end bg-black/50 backdrop-blur-sm" x-data="{ tab: 'overview' }">
        <div class="bg-white w-full md:w-[600px] h-[90vh] md:h-[90vh] overflow-y-auto shadow-2xl animate-slide-in-right p-5 md:p-8 rounded-t-2xl md:rounded-l-2xl relative flex flex-col">
            ...
            {{-- Header --}}
            <div class="flex items-center gap-3 mb-5 shrink-0 mt-2">
                <div class="w-12 h-12 rounded-lg border border-gray-200 p-1 bg-white shadow-sm shrink-0">
                    @if($viewingLinkage->logo_path)
                        <img src="{{ asset('storage/'.$viewingLinkage->logo_path) }}" class="w-full h-full object-contain">
                    @else
                        <div class="w-full h-full bg-gray-50 flex items-center justify-center text-[9px] font-bold text-gray-400">IMG</div>
                    @endif
                </div>
                <div class="flex-1 min-w-0">
                    <h2 class="font-heading font-black text-lg text-gray-900 leading-tight mb-0.5 truncate">{{ $viewingLinkage->name }}</h2>
                    <div class="flex items-center gap-2 text-[10px] text-gray-500 font-bold">
                        <span class="truncate max-w-[120px]">{{ $viewingLinkage->type->name }}</span>
            ...
            <div class="flex gap-6 border-b border-gray-200 mb-5 overflow-x-auto whitespace-nowrap scrollbar-hide shrink-0 pb-1">
                ...
                <button @click="tab = 'activities'" :class="tab === 'activities' ? 'border-red-600 text-red-600' : 'border-transparent text-gray-400'" class="pb-2 text-xs font-bold uppercase border-b-2 transition shrink-0">Activities</button>
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                <div x-show="tab === 'overview'" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 text-xs">
                        <div>
                            <h4 class="font-bold text-gray-400 uppercase mb-1 border-b border-gray-100 pb-1 text-[10px]">Contact</h4>
                            <p class="font-bold text-gray-900 mb-0.5">{{ $viewingLinkage->contact_person ?? 'N/A' }}</p>
                            <p class="text-gray-500 mb-0.5">{{ $viewingLinkage->email }}</p>
                            <a href="{{ $viewingLinkage->website }}" target="_blank" class="text-blue-500 truncate block">{{ $viewingLinkage->website }}</a>
                        </div>
                        <div class="bg-blue-50 p-3 rounded-lg border border-blue-100">
                            <h4 class="font-bold text-blue-800 uppercase mb-2 text-[10px]">Details</h4>
                            <p class="mb-1"><span class="text-blue-600 font-bold">Agreement:</span> {{ $viewingLinkage->agreementLevel->name ?? 'N/A' }}</p>
                            <p class="mb-1"><span class="text-blue-600 font-bold">Start:</span> {{ optional($viewingLinkage->established_at)->format('M d, Y') }}</p>
                            <p><span class="text-blue-600 font-bold">End:</span> {{ optional($viewingLinkage->expires_at)->format('M d, Y') }}</p>
                        </div>
                    </div>

                    <div>
                        <h4 class="text-[10px] font-bold text-gray-400 uppercase mb-2">SDGs</h4>
                ...
                <div x-show="tab === 'activities'" class="space-y-4">
                    ...
                    <div class="space-y-3 pl-2">
                        ...
                        <div class="relative pl-4 border-l-2 border-gray-200">
                            ...
                            <p class="text-[10px] text-gray-600 mt-0.5 line-clamp-2">{{ $activity->description }}</p>
                        </div>
                        @empty
