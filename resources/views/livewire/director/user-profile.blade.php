                        ...
                        <div class="p-6 text-center pt-12 relative">
                            ...
                            <div class="absolute -top-16 left-1/2 transform -translate-x-1/2 w-32 h-32">
                                @php
                                    ...
                                    $photoUrl = $photoPath ? (Str::startsWith($photoPath, ['http', 'images/']) ? asset($photoPath) : asset('storage/' . $photoPath)) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=4F46E5&background=E0E7FF';
                                @endphp
                            ...
                            <h1 class="text-xl md:text-2xl font-black text-gray-900 mt-6 leading-tight">{{ $user->name }}</h1>
                    ...
                    <div class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 relative">
                        ...
                        <div class="p-6 text-center pt-12 relative">
                            ...
                            <div class="absolute -top-16 left-1/2 transform -translate-x-1/2 w-32 h-32">
                                @php
                                    $photoPath = $user->profile_photo_path;
                                    $photoUrl = $photoPath ? (Str::startsWith($photoPath, ['http', 'images/']) ? asset($photoPath) : asset('storage/' . $photoPath)) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=7F9CF5&background=EBF4FF';
                                @endphp
