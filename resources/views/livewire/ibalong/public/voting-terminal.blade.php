<div class="max-w-6xl mx-auto px-4 py-12 space-y-8">
                        ...
                        <div wire:click="selectTeam({{ $team->id }})" 
                            ...
                            <div class="border-t-2 border-dashed {{ $selectedTeamId === $team->id ? 'border-black/30' : 'border-gray-300' }} pt-4 mt-auto">
                                <p class="text-xs font-bold {{ $selectedTeamId === $team->id ? 'text-black/80' : 'text-gray-600' }} line-clamp-3">
                                    {{ $team->description ?? $team->project_description ?? 'No project narrative provided in the logs.' }}
                                </p>
                            </div>
                        </div>
                    @endforeach
    ...
    @if($selectedTeamId)
        @php
            // Extract the selected team cleanly within the Blade file
                            ...
                            @if($activePoll->require_ticket)
                                ...
                                <div class="bg-gray-50 p-4 border-2 border-dashed border-gray-300 relative">
                                    ...
                                    <div class="flex gap-2 mb-3">
                                        ...
                                        {{-- Open Scanner Button --}}
                                        <button type="button" onclick="initQRScanner()" class="bg-iba-black text-white px-4 border-2 border-iba-black hover:bg-iba-teal transition-colors flex items-center justify-center" title="Scan or Upload QR Ticket">
                                    ...
                                    <div wire:ignore>
                                        <div id="qr-reader" style="display: none;" class="w-full border-2 border-iba-teal p-2 bg-white text-xs"></div>
                                    </div>
                                </div>
        ...
        {{-- Javascript for QR Scanner Protocol --}}
        <script>
            ...
            function initQRScanner() {
                ...
                html5QrcodeScanner.render(
                    function onScanSuccess(decodedText) {
                        // Push decoded value to Livewire backend
                        @this.set('ticketCode', decodedText);
                        
                        // Close the scanner UI automatically
                        html5QrcodeScanner.clear();
                        document.getElementById('qr-reader').style.display = 'none';
                    }, 
                    function onScanFailure(error) {
                        // Suppress background scanning errors
                    }
                );
            }

            // Listen for Livewire canceling the modal to safely shut down the camera
