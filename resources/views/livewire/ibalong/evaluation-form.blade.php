<div class="max-w-4xl mx-auto space-y-8 pb-24">
    ...
    @if($isSubmitted)
        <div class="bg-white border-4 border-iba-black shadow-[8px_8px_0_0_#0095AC] p-12 text-center relative overflow-hidden">
    ...
    @else
        ...
        <div class="bg-white border-4 border-iba-black shadow-[8px_8px_0_0_#FF8623] p-6 relative">
            ...
            <h1 class="text-2xl font-black text-iba-black uppercase tracking-wider mt-2">{{ $evaluation->title }}</h1>
            <p class="text-sm font-bold text-gray-700 leading-relaxed whitespace-pre-wrap mt-4 border-t-2 border-dashed border-gray-300 pt-4">{{ $evaluation->description }}</p>
        </div>

        {{-- The Dynamic Questions --}}
        <form wire:submit.prevent="submit" class="space-y-6">
            @foreach($evaluation->questions as $task)

                {{-- Section Headers --}}
                @if($task->type === 'section')
                    <div class="bg-iba-black text-white border-4 border-iba-black shadow-[6px_6px_0_0_#131011] p-6 mt-12">
                        <h2 class="text-xl font-black uppercase tracking-widest">{{ $task->question_text }}</h2>
                        @if($task->description)
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mt-2">{{ $task->description }}</p>
                        @endif
                    </div>

                {{-- Page Breaks (Visual Dividers in continuous scroll) --}}
                @elseif($task->type === 'page_break')
                    <div class="flex items-center gap-4 py-4">
                        <div class="flex-1 border-t-4 border-dashed border-gray-300"></div>
                        <span class="text-[10px] font-black uppercase text-gray-400 tracking-widest">End of Sector</span>
                        <div class="flex-1 border-t-4 border-dashed border-gray-300"></div>
                    </div>
                ...
                @else
                    <div class="bg-white border-4 border-iba-black shadow-[4px_4px_0_0_#131011] p-6 flex flex-col gap-3">
                        ...
                        @if($task->description)
                            <p class="text-xs font-bold text-gray-500">{{ $task->description }}</p>
                        @endif
                        ...
                        @elseif($task->type === 'dropdown')
                            <select wire:model="answers.{{ $task->id }}" class="w-full border-2 border-iba-black p-3 font-bold focus:outline-none focus:border-iba-orange bg-gray-50">
                            ...
                            <div class="flex flex-col gap-3 mt-2">
                                @foreach($task->options as $option)
                                    ...
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="radio" wire:model="answers.{{ $task->id }}" value="{{ trim($optText) }}" class="w-5 h-5 text-iba-orange border-2 border-iba-black focus:ring-0">
                                        <span class="text-xs font-bold uppercase group-hover:text-iba-orange transition-colors">{{ trim($optText) }}</span>
                                    </label>
                                @endforeach
                            </div>
                        ...
                        @elseif($task->type === 'checkbox')
                            <div class="flex flex-col gap-3 mt-2">
                                @foreach($task->options as $option)
                                    @php $optText = is_array($option) ? $option['text'] : $option; @endphp
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" wire:model="answers.{{ $task->id }}" value="{{ trim($optText) }}" class="w-5 h-5 text-iba-teal border-2 border-iba-black focus:ring-0">
                                        <span class="text-xs font-bold uppercase group-hover:text-iba-teal transition-colors">{{ trim($optText) }}</span>
                                    </label>
                                @endforeach
                            </div>
                                ...
                                <div class="flex gap-4 min-w-max border-2 border-iba-black bg-gray-50 p-4">
                                    @foreach($task->options as $option)
                                        <label class="flex flex-col items-center gap-2 cursor-pointer group w-24">
                                            <span class="text-[9px] font-black uppercase text-center text-gray-500 group-hover:text-iba-black h-8 flex items-end justify-center">{{ is_array($option) ? $option['text'] ?? '' : $option }}</span>
            ...
            <div class="pt-6 flex justify-end">
                <button type="submit" class="bg-iba-orange text-iba-black text-sm font-black uppercase tracking-widest px-12 py-4 border-4 border-iba-black shadow-[6px_6px_0_0_#131011] hover:translate-y-1 hover:shadow-none transition-all relative" wire:loading.attr="disabled">
                    <span wire:loading.remove wire:target="submit">Transmit Data</span>
                    <span wire:loading wire:target="submit" class="animate-pulse">Processing...</span>
                </button>
            </div>
        </form>
    @endif
