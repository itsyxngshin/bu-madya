...
>
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 custom-scrollbar">
        ...
        @if($isAdmin)
            ...
            @php $isPillarsActive = request()->routeIs('director.pillars.*'); @endphp
            <a href="{{ route('director.pillars.index') }}" class="{{ $linkClass }} {{ $isPillarsActive ? $activeClass : $inactiveClass }}">
