...
class PublicElectionResults extends Component
{
    ...
    public function render()
    {
        ...
        // THE FIX: Explicitly load the election's political parties
        $this->election->loadMissing('parties');

        // Fetch positions and approved candidates.
        // THE FIX: Added 'party' to the candidate eager loading array.
        $positions = $this->election->positions()->with(['candidates' => function($query) {
            $query->where('status', 'approved')
                  ->with(['user', 'college', 'party']) // <-- ADDED 'party' HERE
